...
class Usuario:
    ...
    def get_all(cls):
        ...
        query = "SELECT id, nombre, apellido, email, contraseña, created_at, updated_at FROM usuarios;"
    ...
    def existe_correo(cls, correo):
        ...
        resultados = connectToMySQL('core').query_db(query, data)
    ...
    def get(cls, id):
        ...
        query = f"SELECT id, nombre, apellido, email, contraseña, created_at, updated_at FROM usuarios WHERE id = %(id)s;"
        data = {
            'id': id
        }
        resultados = connectToMySQL('core').query_db(query, data)
    ...
    def get_by_email(cls, email):
        ...
        query = f"SELECT id, nombre, apellido, email, contraseña, created_at, updated_at FROM usuarios WHERE email = %(email)s;"
        data = {
            'email': email
        }
        resultados = connectToMySQL('core').query_db(query, data)
    ...
    def save(cls, datos):
        ...
        return connectToMySQL('core').query_db(query, datos)
    ...
    def delete(cls, id):
        ...
        connectToMySQL('core').query_db(query, data)
        return True